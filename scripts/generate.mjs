...
<div class="hero-strip rv in d3" role="group" aria-label="Drei Bereiche"><a href="/entruempelung/">${pic('svc-entruempelung-hero', { alt: 'Kartons werden aus dem vollen Keller getragen — Entrümpelung im Havelland', sizes: '(max-width:760px) 30vw, 200px' })}<span>Entrümpelung</span></a><a href="/zaunbau/">${pic('svc-zaunbau-hero', { alt: 'Zaunpfosten wird mit der Wasserwaage ausgerichtet', sizes: '(max-width:760px) 30vw, 200px' })}<span>Zaunbau</span></a><a href="/fuer-hausverwaltungen/">${pic('svc-hausmeisterservice-hero', { alt: 'Hausmeister mit Werkzeugkoffer vor dem Hauseingang — Objektpflege im Havelland', sizes: '(max-width:760px) 30vw, 200px' })}<span>Objektpflege</span></a></div>
</div></section>
${gstripHome}
