...
<section class="sec"><div class="wrap"><div class="prose wide rv"><h2>Ein fester Ansprechpartner — ${esc(nap.inhaber)}</h2><p>Hinter dem Haus- &amp; Gartenservice Havelland steht ${esc(nap.inhaber)} als persönlicher Ansprechpartner. Sie reden vom ersten Anruf bis zur Abnahme mit derselben Person — kein Callcenter, keine wechselnden Kräfte, keine Warteschleife. Das macht Absprachen einfach: Was besprochen ist, wird so umgesetzt, und bei Rückfragen wissen Sie, an wen Sie sich wenden.</p><p>Der Betrieb sitzt in der ${esc(nap.street)} in ${esc(nap.zip)} ${esc(nap.city)} und ist im gesamten Havelland und Berliner Umland unterwegs. Als Kleinunternehmer nach § 19 UStG arbeiten wir schlank und ohne Mehrwertsteuer-Aufschlag — der Festpreis, den Sie nach der Besichtigung bekommen, ist der Endpreis.</p></div></div></section>
...
${endBand}`;
  write(url, head(`Über uns — ${nap.name}`, mkMeta(`Haus- & Gartenservice Havelland: regionaler Betrieb aus ${nap.city}, Inhaber ${nap.inhaber}. Ein fester Ansprechpartner, Festpreis ohne Mehrwertsteuer, Foto-Nachweis.`), url, `${orgSchema()},${breadcrumb([{name:'Start',url:'/'},{name:'Über uns',url}])}`) + header + main + footer + SCTA_DEFAULT + revealJS + '</body></html>');
  written.basis.push(url);
}
