...
function b2bPage() {
  ...
  const leist = leistDetail.length
    ? leistDetail.map((l, i) => `<a class="card" href="/${esc(l.link_to)}/"><span class="n">${String(i + 1).padStart(2, '0')}</span><h3>${esc(l.h3)}</h3><p>${esc(l.body)}</p><span class="go">${esc(l.label || 'Mehr erfahren')} →</span></a>`).join('')
  ...
  const ablauf = (b2b.ablauf && Array.isArray(b2b.ablauf.schritte) && b2b.ablauf.schritte.length)
    ? `<section class="sec"><div class="wrap"><div class="prose wide rv"><h2>${esc(b2b.ablauf.h2 || 'So kommen Sie zum Angebot')}</h2><ol>${b2b.ablauf.schritte.map(x => `<li>${esc(x)}</li>`).join('')}</ol></div></div></section>` : '';
